...
def get_generated_inception_stats(G, Z_sampler, inv_transform, size, batch_size=16, device='cuda'):
    dims = 2048
    ...
    for i in tqdm(range(0, size, batch_size)):
        ...
        batch = G_Z.detach().type(torch.FloatTensor).to(device=device)
        pred = model(batch)[0]
...
def energy_based_distance(X, Y, n_projections=10000, biased=False, device='cuda'):
    '''
    ...
    thetas = torch.randn(n_projections, X.size(1)).to(device=device)
    ...
    I = torch.ones(idx.size(), dtype=torch.float32, device=device) / X.size(0)
